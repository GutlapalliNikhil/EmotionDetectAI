...
@try_export
def export_torchscript(model, im, file, optimize, interpreter_version=5, prefix=colorstr('TorchScript:')):
    # YOLOv5 TorchScript model export
    LOGGER.info(f'\n{prefix} starting export with torch {torch.__version__}...')
    f = file.with_suffix('.torchscript')
    fl = file.with_suffix('.torchscript.ptl')
    
    nik_model = torch.quantization.convert(model)
    nik_model_1 = torch.jit.script(nik_model)
    nik_model_2 = torch.utils.optimize_for_mobile(nik_model_1)
    torch.jit.save(nik_model_2, "testing_yolov5.ptl")
    print("Nik")
    exit()
    ts = torch.jit.trace(model, im, strict=False)
    d = {'shape': im.shape, 'stride': int(max(model.stride)), 'names': model.names}
    extra_files = {'config.txt': json.dumps(d)}  # torch._C.ExtraFilesMap()
    if optimize:
        scripted_model = optimize_for_mobile(ts)
        scripted_model._save_for_lite_interpreter(str(f), _extra_files=extra_files)
        scripted_model.save(f)
        scripted_model._save_for_lite_interpreter(str(fl))
    else:
        ts.save(str(f), _extra_files=extra_files)
    return f, None
